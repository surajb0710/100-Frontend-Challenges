...
const ECommerceProductCard = () => {
  ...
  const handleImageClick = (imageIndexNumber) => {
    setCurrentIndexNumber(imageIndexNumber);
  };

  const handleSelectSizeClick = (selectedSize) => {
    ...
    <div className={style.container}>
      <div className={style.imagesContainer}>
        <div className={style.imagesCarousal}>
          {images.map((image, imageIndexNumber) => (
            <div className={style.carousalImageContainer} key={image.id}>
              <img
                src={image.url}
                className={style.carousalImage}
                onClick={() => handleImageClick(imageIndexNumber)}
              />
            </div>
          ))}
        </div>
      ...
      <div className={style.productDetailsContainer}>
        ...
        <h1 className={style.productName}>Custom Fit Polo Bear Oxford Shirt</h1>
        ...
        <div className={style.sizeOptionsContainer}>
          {availableSize.map((size) => (
            <p
              key={size}
              className={`${style.availableSize} ${
                size === selectedSize ? style.selectedSize : ''
              }`}
              onClick={() => handleSelectSizeClick(size)}
            >
              {size}
            </p>
          ))}
